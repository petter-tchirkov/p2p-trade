<template>
  <div class="review pl-56 w-full max-h-screen h-full">
    <header class="review__header flex justify-between p-3 pl-8 border-b-2">
      <h1>Отзывы о {{users[$route.params.id].name}}</h1>
      <UserPanel/>
    </header>
    <div class="review__content">
      ...
      <div class="review__user user-info pt-5 pl-11 border-r-2 border-b-2">
        <div class="user-info__top flex items-center mb-5">
          <img class="mr-5" :src="require(`~/assets/images/people/${users[$route.params.id].id}.png`)" alt="">
          <h2 class="mr-3">{{users[$route.params.id].name}} {{users[$route.params.id].id}}</h2>
          <img v-if="users[$route.params.id].status === 'verified'" src="~/assets/images/verified.svg" alt="">
        </div>
        <div class="user-info__grid">
          <div class="user-info__row mb-4">
            <span class="user-info__criteria">Статус</span>
            <span class="user-info__status w-max px-2 py-1">
              {{users[$route.params.id].status === 'verified' ? 'Верифицирован' : 'Не верифицирован'}}
            </span>
          </div>
          <div class="user-info__row mb-4">
            <span class="user-info__criteria">Был на сайте</span>
            <span :class="users[$route.params.id] ? 'user-info__is-online text-green-400 pl-4 relative text-green' : 'user-info__is-online text-red-600 pl-4 relative'">
              {{users[$route.params.id].isOnline ? 'Online' : 'Offline'}}
            </span>
          </div>
          <div class="user-info__row mb-4">
            <span class="user-info__criteria">О себе</span>
            <span class="user-info__description pr-12">
              {{users[$route.params.id].description}}
            </span>
          </div>
          <div class="user-info__row mb-4">
            <span class="user-info__criteria">На проекте</span>
            <span class="user-info__description">
              {{users[$route.params.id].since}}
            </span>
          </div>
          <div class="user-info__row mb-4">
            <span class="user-info__criteria">Активных заявок</span>
            <span class="user-info__description">
              {{users[$route.params.id].activeOrders}}
            </span>
          </div>
          <div class="user-info__row mb-4">
            <span class="user-info__criteria">Среднее время отправки депозита</span>
            <span class="user-info__description">
              {{users[$route.params.id].averageDepositTime}} мин
            </span>
          </div>
          <div class="user-info__row mb-4">
            <span class="user-info__criteria">В черном списке у</span>
            <span class="user-info__description">
              {{users[$route.params.id].blackListed}} пользователей
            </span>
          </div>
        </div>
      </div>
